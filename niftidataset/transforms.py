...
class RandomBlock:
    ...
    def block2d(self, src, tgt):
        ...
        if random.random() < self.p:
            if self.tfm_x: src[:, h - sh // 2:h + sh // 2 + oh, w - sw // 2:w + sw // 2 + ow] = np.random.uniform(
                *int_range)
            if self.tfm_y: tgt[:, h - sh // 2:h + sh // 2 + oh, w - sw // 2:w + sw // 2 + ow] = np.random.uniform(
                *int_range)
        return src, tgt
    ...
    def block3d(self, src, tgt):
        ...
        if random.random() < self.p:
            if self.tfm_x: src[:, h - sh // 2:h + sh // 2 + oh, w - sw // 2:w + sw // 2 + ow,
                           d - sd // 2:d + sd // 2 + od] = np.random.uniform(*int_range)
            if self.tfm_y: tgt[:, h - sh // 2:h + sh // 2 + oh, w - sw // 2:w + sw // 2 + ow,
                           d - sd // 2:d + sd // 2 + od] = np.random.uniform(*int_range)
        return src, tgt
